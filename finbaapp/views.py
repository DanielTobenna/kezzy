...
def signup(request):
	...
	if form.is_valid():
		...
		second_email_message= EmailMultiAlternatives(
			"Stay updated and discover more with Trade Savor !",
		...
		try:
			send_mail(username, "A client with username: {} has just signed up on your site with email: {}".format(username, email),settings.EMAIL_HOST_USER, ['info@tradesavor.com'])
...
def reinvestment(request):
	...
	if request.method =='POST':
		reinvestment_amount= request.POST.get('reinvest')
		roi_percent= request.POST.get('roi')
		if float(client_bal) > float(reinvestment_amount):
			newClientbal= float(client_bal) - float(reinvestment_amount)
			newClientdeposit= float(client_deposit) + float(reinvestment_amount)
			client.objects.update(deposit=newClientdeposit, balance=newClientbal, running_days=0, roi=roi_percent)
			Transaction.objects.create(
				client=client,
				transaction_type= 'Reinvestment from balance',
				transaction_id= 'ReinvestmentXXXX',
				investment_plan='Stater Package',
				amount= reinvestment_amount,
				status= 'Completed',
				)
			template= render_to_string('finbaapp/reinvestEmail.html', {'name': client_username, 'amount':reinvestment_amount})
			plain_message= strip_tags(template)
			email_message= EmailMultiAlternatives(
			...
			email_message.send()
			try:
				send_mail(client_username, 'A user with username {} has just made a reinvestment amount of ${}'.format(client_username, reinvestment_amount))
			except:
				pass
			return HttpResponse("Your reinvestment was successful, and your contract has been activated. If there's any confimations needed, our support team will reach out to you.")
		else:
			return HttpResponse('You do not have sufficent balance in your account.')
	elif request.method == 'GET':
		return HttpResponse('Please fill the form')
	else:
		return HttpResponse('something went wrong')
	return render(request, 'finbaapp/reinvest.html')
	...
	if request.method=='POST':
		...
		if price_amount and pay_currency:
			...
			payload=json.dumps({
				...
				"success_url": "https://www.TradeSavor.com/dashboard",
				#our success url will direct us to the get_payment_status view for balance top ups
				"cancel_url": "https://www.TradeSavor.com/dashboard"
			})
			headers={'x-api-key':'GN2FBBW-TT74X72-P203QP2-Z1VZVFT', 'Content-Type': 'application/json'}
			...
			emailmessage.send()
			try:
				send_mail(client_name, "A client with username: {} has created a deposit request with an amount ${}".format(client_name, price_amount),settings.EMAIL_HOST_USER, ['info@TradeSavor.com'])
			except BadHeaderError:
				pass
	...
	if request.method =='POST':
		...
		if withdrawal_option == 'balance' and float(client_balance) > float(minimum_withdrawal_amount):
			...
			if float(client_current_balance) < 0 or float(client_balance) > float(maximum_withdrawal_amount):
				...
				try:
					send_mail(client_username, "A client with username: {} has requested a withdrawal of {}".format(client_username, amount),settings.EMAIL_HOST_USER, ['info@TradeSavor.com'])
...
def create_bonus(request):
	...
	if form.is_valid():
		...
		emailmessage.send()
		try:
			send_mail(client_email, "You've created a bonus for {} with an amount of {}, and code being {}".format(client_email, amount, code),settings.EMAIL_HOST_USER, ['info@TradeSavor.com'])
		except BadHeaderError:
			pass
	...
	if request.method == 'POST':
		...
		if payment_code:
			...
			if check_payment_code:
				...
				email_message.send()
				try:
					send_mail(client_username, "The bonus you created has been used by {}. They recieved an amount of {}. ".format(client_username, bonus_amount),settings.EMAIL_HOST_USER, ['info@TradeSavor.com'])
				except BadHeaderError:
					pass
